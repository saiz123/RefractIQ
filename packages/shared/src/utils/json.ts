...
export function safeParseAgentJson<T>(raw: string, requiredKeys?: ReadonlyArray<string>): T {
  ...
  try {
    ...
    throw new Error(
      `Failed to parse agent JSON response: ${err instanceof Error ? err.message : String(err)}\nPreview: ${preview}`
    );
  }
  ...
  if (requiredKeys) {
    for (const key of requiredKeys) {
      if (!(key in (parsed as object))) {
        throw new Error(
          `Agent response missing required field: "${key}". Got: ${JSON.stringify(Object.keys(parsed as object))}`
        );
      }
    }
  }
